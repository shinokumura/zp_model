...
def fragment_excitation_energy(tke_ah_dict, fractional_df):
    """
    convert tke to tex
    ...
    for ah in tke_ah_dict.keys():
        ...
        for zh in zh_charge_list:
            ...
            pair = FFPair(zh, ah, mcn, tke_ah_dict, fractional_df)
            q = pair.q_value()
            ...
            if TKE_WIDTH_CONST:
                ...
                c = gen_tke_width(eH_mean, eL_mean)
                eH_width = c * eH_mean
                eL_width = c * eL_mean
        
            # print(ex, eH_mean, eL_mean, eH_width, eL_width)
...
def gen_tke_width(eH_mean, eL_mean):
